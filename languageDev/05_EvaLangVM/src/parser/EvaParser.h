...
struct Exp {
  ...
  // Numbers:
  inline Exp(int number) : type(ExpType::NUMBER), number(number) {}

  // Strings, Symbols:
  inline Exp(std::string& strVal) {
    if (strVal[0] == '"') {
      type = ExpType::STRING;
  ...
  // Lists:
  inline Exp(std::vector<Exp> list) : type(ExpType::LIST), list(list) {}
...
class Tokenizer {
   public:
     ...
     */
    inline bool hasMoreTokens() { return cursor_ <= static_cast<size_t>(str_.length()); }

    /**
     * Returns current tokenizing state.
     */
    inline TokenizerState getCurrentState() { return states_.back(); }

    /**
     * Enters a new state pushing it on the states stack.
     */
    inline void pushState(TokenizerState state) { states_.push_back(state); }

    /**
     * Alias for `push_state`.
     */
    inline void begin(TokenizerState state) { states_.push_back(state); }

    /**
     * Exits a current state popping it from the states stack.
     */
    inline TokenizerState popState() {
     ...
     */
    inline bool isEOF() { return cursor_ == static_cast<size_t>(str_.length()); }

    inline SharedToken toToken(TokenType tokenType) {
   ...
   private:
     ...
     */
    inline void captureLocations_(const std::string& matched) {
        ...
        std::string lineStr;
        std::getline(ss, lineStr, '\n');
        while (ss.tellg() > 0 && static_cast<size_t>(ss.tellg()) <= len) {
            currentLine_++;
